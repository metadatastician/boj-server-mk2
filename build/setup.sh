...
# setup.sh — Universal setup script for boj-server-mk2
#
...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/metadatastician/boj-server-mk2/main/setup.sh -o setup.sh && sh setup.sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== boj-server-mk2 Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/metadatastician/boj-server-mk2.git"
        info "Then: cd boj-server-mk2 && ./setup.sh"
        exit 1
    fi
