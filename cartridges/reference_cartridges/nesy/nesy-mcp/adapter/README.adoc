...
= nesy-mcp / adapter — protocol bridge
:orientation: shallow
...
| `nesy_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `nesy_adapter.zig` — dispatch function and per-protocol routers.
