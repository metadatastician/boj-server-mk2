...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libnesy.so`.
| `nesy_ffi.zig`     | C-ABI exports (11 exports, 9 inline tests, 259 lines).
|===
...
9 inline `test "..."` block(s) in `nesy_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `nesy_ffi.zig` — C-ABI signatures and guards.
