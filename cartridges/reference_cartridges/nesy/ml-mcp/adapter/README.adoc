...
= ml-mcp / adapter — protocol bridge
:orientation: shallow
...
| `ml_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `ml_adapter.zig` — dispatch function and per-protocol routers.
