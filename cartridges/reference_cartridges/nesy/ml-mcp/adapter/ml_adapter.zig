...
//
// ml-mcp/adapter/ml_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9265), gRPC-compat (port 9266),
// GraphQL (port 9267).
// Replaces the banned zig adapter (ml_adapter.v).

const std = @import("std");
const ffi = @import("ml_ffi");

const REST_PORT: u16 = 9265;
const GRPC_PORT: u16 = 9266;
const GQL_PORT:  u16 = 9267;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "ml_authenticate")) {
        return .{ .status = 200, .body = okJson(resp, "ml_authenticate forwarded") };
    }
    if (std.mem.eql(u8, tool, "ml_inference")) {
        return .{ .status = 200, .body = okJson(resp, "ml_inference forwarded") };
    }
    if (std.mem.eql(u8, tool, "ml_list_models")) {
        return .{ .status = 200, .body = okJson(resp, "ml_list_models forwarded") };
    }
    if (std.mem.eql(u8, tool, "ml_get_model_info")) {
        return .{ .status = 200, .body = okJson(resp, "ml_get_model_info forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "ml_authenticate") != null)
        return dispatch("ml_authenticate", body, resp);
    if (std.mem.indexOf(u8, body, "ml_inference") != null)
        return dispatch("ml_inference", body, resp);
    if (std.mem.indexOf(u8, body, "ml_list_models") != null)
        return dispatch("ml_list_models", body, resp);
    if (std.mem.indexOf(u8, body, "ml_get_model_info") != null)
        return dispatch("ml_get_model_info", body, resp);
