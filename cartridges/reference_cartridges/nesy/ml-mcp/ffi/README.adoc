...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libml.so`.
| `ml_ffi.zig`     | C-ABI exports (20 exports, 12 inline tests, 468 lines).
|===
...
12 inline `test "..."` block(s) in `ml_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `ml_ffi.zig` — C-ABI signatures and guards.
