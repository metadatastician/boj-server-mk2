...
= claude-ai-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libclaude-ai.so`.
| `claude_ai_mcp_ffi.zig`     | C-ABI exports (2 exports, 0
0 inline tests, 15 lines).
|===
...
0
0 inline `test "..."` block(s) in `claude_ai_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `claude_ai_mcp_ffi.zig` — C-ABI signatures and guards.
