...
# BOJ_SERVER_MK2 ABI/FFI Documentation
...
│  generated/abi/boj_server_mk2.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libboj_server_mk2.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── boj_server_mk2.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── boj_server_mk2.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/boj_server_mk2.h
```
...
```c
#include "boj_server_mk2.h"

int main() {
    void* handle = boj_server_mk2_init();
    if (!handle) return 1;

    int result = boj_server_mk2_process(handle, 42);
    if (result != 0) {
        const char* err = boj_server_mk2_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    boj_server_mk2_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lboj_server_mk2 -L./zig-out/lib
```

### From Idris2

```idris
import BOJ_SERVER_MK2.ABI.Foreign
...
```rust
#[link(name = "boj_server_mk2")]
extern "C" {
    fn boj_server_mk2_init() -> *mut std::ffi::c_void;
    fn boj_server_mk2_free(handle: *mut std::ffi::c_void);
    fn boj_server_mk2_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = boj_server_mk2_init();
        assert!(!handle.is_null());

        let result = boj_server_mk2_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libboj_server_mk2 = "libboj_server_mk2"

function init()
    handle = ccall((:boj_server_mk2_init, libboj_server_mk2), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:boj_server_mk2_process, libboj_server_mk2), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:boj_server_mk2_free, libboj_server_mk2), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/boj_server_mk2.h
   ```
