...
_Note: All cartridges must invert control back to the BEAM host for side-effects. See Section 6._
...
. The Elixir Host handles the network/API request.
. The Elixir Host passes the data payload to the SNIF.
. The Idris2 SNIF performs pure computation over the data and returns a Cap'n Proto "Action Blueprint" (e.g., `{"command": "send_email", "to": "..."}`).
. The Elixir Host executes the command on the SNIF's behalf.
